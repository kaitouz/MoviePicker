describe('Test authorized', () => {
  ...
  it('test_delete_bookmark', function() {
    cy.get('.user-config').should('be.visible').click()
    cy.get('.dropdown-list').get('a').contains('Bookmarks').click({force: true})
    cy.location('pathname').should('eq', '/bookmark') 
    cy.wait(5000)
    cy.log(typeof(movie_title));
    cy.get('.movie-card_container > .title').should('contain.text', movie_title);
    cy.wait(5000)

    cy.get('.bookmark__star').click();
    cy.wait(5000)
