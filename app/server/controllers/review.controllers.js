...
exports.editReview = async (req, res) => {
    ...
    if (review.user_id !== user.id) 
        return res
            .status(401)
            .send('You have no right to edit this review')

    const result = await reviewService.updateReview(review_id, req.body.new_content)
    res.json({
...
exports.movieReview = async (req, res) => {
    if (!req.query.movie_id) 
        return res.status(401).send('movie_id not found')

    const results = await reviewService.getAllReviewsByMovieID(req.query.movie_id)
