...
exports.rateMovie = async (req, res) => {
    ...
    const rating = await ratingService.getRatingByUserAndMovie(user.id, movie_id)
    if (!rating) {
        await ratingService.addRating(user.id, movie_id, score)
        const result = await ratingService.getRatingByUserAndMovie(user.id, movie_id)
        res.json({
...
exports.movieRating = async (req, res) => {
    ...
    const results = await ratingService.getAllRatingsByMovieId(req.query.movie_id)
    res.send(results)
}
