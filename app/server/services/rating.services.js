...
const addRating = async (user_id, movie_id, score) => await query(
    "INSERT INTO rating (user_id, movie_id, score) VALUES (?, ?, ?)",
    [user_id, movie_id, score]
)


const updateRating = async (rating_id, score) => await query(
    "UPDATE rating SET score = ? WHERE id = ?",
    [score, rating_id]
)


const getAllRatingsByMovieId = async (movie_id) => await query(
    "SELECT id, user_id, score FROM rating WHERE movie_id = ?",
    movie_id
)


const getAllRatingsByUserID = async (user_id) => await query(
